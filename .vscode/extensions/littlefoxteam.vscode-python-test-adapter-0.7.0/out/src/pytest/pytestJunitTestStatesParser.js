...
function mapToTestState(testcase, cwd) {
    ...
    return {
        ...
        test: testId,
        type: 'test',
        message: strings_1.concatNonEmpty(os_1.EOL + os_1.EOL, message, output),
        decorations,
...
function getTestState(testcase) {
    const output = strings_1.concatNonEmpty(os_1.EOL, extractSystemOut(testcase), extractSystemErr(testcase));
...
function extractSystemOut(testcase) {
    return collections_1.empty(testcase['system-out']) ? '' : testcase['system-out'].join(os_1.EOL);
}
function extractSystemErr(testcase) {
    return collections_1.empty(testcase['system-err']) ? '' : testcase['system-err'].join(os_1.EOL);
}
function extractErrorMessage(errors) {
    if (!errors || !errors.length) {
        return '';
    }
    return strings_1.concatNonEmpty(os_1.EOL, ...errors.map(e => strings_1.concatNonEmpty(os_1.EOL, e.$.message, e._)));
}
...
function matchModule(testClass, testFile) {
    ...
    if (strings_1.startsWith(testClass, name, position)) {
        let moduleEndPosition = position + name.length;
        if (ext && strings_1.startsWith(testClass, ext, moduleEndPosition)) {
            moduleEndPosition += ext.length;
        }
        if (strings_1.startsWith(testClass, '.', moduleEndPosition)) {
            moduleEndPosition += 1;
        }
...
function matchParentPath(testClass, testFile) {
    ...
    const allClassPartsMatchesPath = testFileParentPath.every(pathPart => {
        if (strings_1.startsWith(testClass, pathPart + '.', index)) {
